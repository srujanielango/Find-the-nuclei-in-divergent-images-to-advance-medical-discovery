...
from network import *
from image_reader import ImageReader, decode_labels, inv_preprocess, prepare_label, write_log, read_labeled_image_list
...
class Model(object):
	...
	def predict(self):
		...
		for step in range(self.conf.test_num_steps):
			...
			img_name = image_list[step].split('/')[5].split('.')[0]
			#print(img_name)
	...
	def train_setup(self):
		...
		with tf.name_scope("create_inputs"):
			reader = ImageReader(
				...
				IMG_MEAN,
				self.coord)
			self.image_batch, self.label_batch = reader.dequeue_up_to(self.conf.batch_size)
